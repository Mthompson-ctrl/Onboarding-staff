...
- Next: **Phase 3 M4** — upload form UI inside the dialog body. Per-doc-type fields rendered from `document_types` flags; submit calls a stubbed action returning `{ error: "not implemented" }` so the wiring (form rendering, client-side validation, field-error display, dialog close behaviour) is verified before Storage goes live in M5. HR admin login flow + `organisation_members` seed comes after Phase 3.
