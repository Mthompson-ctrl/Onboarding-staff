...
- Candidate portal **Phase 3** in progress — `/portal/documents` (list + upload).
  ...
  - **M5 landed and verified end-to-end against dev** with a real 67 KB PDF upload by the seeded candidate: `documents` row created with full `<org>/<candidate>/<code>/<doc_id>.pdf` storage_path, status defaulted to `pending_review`, all NOT NULL columns populated; matching `audit_log` row with `action='documents.insert'`, `actor_type='candidate'`, and `entity_id` aligned with both `documents.id` and the storage filename (three sources of truth in lockstep). Dialog closed cleanly via the close-after-success choreography; the list page re-rendered showing "Pending review" + "Uploaded 8 May 2026", and the Upload button hid via the existing list-page conditional. The full pipeline now lives in `src/app/portal/documents/actions.ts`: `auth.getUser()` → `candidates` lookup by `user_id` → `document_types` lookup (system-global + active) → `documentUploadSchema(flags).safeParse` → `supabase.storage.upload()` (bucket `candidate-documents`, `upsert: false`) → `supabase.rpc('candidate_insert_document', ...)` → best-effort orphan-blob `.remove()` on RPC failure → SQLSTATE `23505` mapped to a friendly "already uploaded" message → `{ ok: true }` on success. `next.config.ts` lifts `experimental.serverActions.bodySizeLimit` to `"6mb"` (Next 16 only supports a global value — no per-route scoping today; the 5 MB PDF cap stays enforced in the validation schema and bucket config). Dialog client component (`upload-dialog.tsx`) gains a hidden `name="code"` input pinned to the row's doc-type code, plus a `useEffect` on `state.ok` (guarded by `didCloseRef`) that calls `setOpen(false)` + `router.refresh()`.
  ...
  - **Phase 3 design note — close-after-success choreography** (decided in M3, landed in M5): `uploadDocument` returns `{ ok: true } | { fieldErrors } | { error }` rather than calling `redirect()`. `redirect()` throws `NEXT_REDIRECT`, which bypasses any `setOpen(false)` queued client-side. The dialog reacts to `ok` in a `useEffect` (guarded by `didCloseRef` so re-mounts with retained ok-state can't re-fire), calling `setOpen(false)` + `router.refresh()`.
- Next: **HR admin login flow + `organisation_members` seed**, then HR-side document review (verify / reject) for Phase 4. Re-upload / supersession via `replaces_document_id` is still deferred — the RPC currently always inserts as a new document and the unique partial index will fire `23505` on a second attempt for the same type (mapped to a friendly message in the action).
