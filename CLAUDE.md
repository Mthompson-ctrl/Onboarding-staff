...
- **Phase 3 design note — close-after-success choreography:** when the M3/M4 form goes live inside the dialog, a successful upload needs to close the modal, refresh the list, and surface a "Pending review" toast. `redirect()` from a server action throws `NEXT_REDIRECT` which bypasses any `setOpen(false)` you queued after `await` — pick a deliberate pattern in M3 (action returns `{ ok: true }` + client closes via `setOpen(false)` + `router.refresh()`, OR action calls `revalidatePath` server-side and returns, client closes). Don't redirect from inside the dialog action.
- Next: **Phase 3 M3** — validation schema (`src/lib/validation/document.ts`) and helpers (path builder, MIME / size constants in `src/lib/documents.ts`). Pure code, no UI. HR admin login flow + `organisation_members` seed comes after Phase 3.
