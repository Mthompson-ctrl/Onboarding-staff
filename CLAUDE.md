...
│   └── mobile/        — Capacitor wrap around a Next.js static export
│                        (output: "export"). Candidate-only scope; no HR
│                        admin, no public enquiry form. Imports from
│                        @sentinel/shared. Android scaffolded in M2
│                        (Capacitor 7.6.4); iOS scaffold deferred to a
│                        Mac session — see apps/mobile/README.md. Auth
│                        via @supabase/supabase-js (localStorage) lands
│                        in Phase Mobile M3.
├── packages/
...
`apps/mobile` is a Capacitor wrap around a Next.js static export, candidate-only scope. Shares utilities with `apps/web` via `@sentinel/shared`. Android scaffold lives under `apps/mobile/android/`; iOS scaffold deferred to a Mac session (invocation in `apps/mobile/README.md`).

- **Capacitor pinned to `^7.0.0`** (7.6.4 installed; 8.3.3 available). Bump to 8.x **before Phase Mobile M6** (camera plugin work) so any plugin-compatibility issues surface during integration, not at store submission.
- **Web/mobile typography deliberately NOT unified.** Mobile uses a system font stack (San Francisco on iOS, Roboto on Android) — saves ~100 KB of WOFF2, renders instantly, matches native platform conventions on each device. Web continues with Geist for brand consistency. Don't "unify" without reading the M2.2 commit first.
- **Anon-key invariant.** The mobile bundle ships to phones; anything in `process.env` at build time is baked into the JavaScript distributed inside the APK / IPA, where it can be inspected by anyone who unpacks the artefact. Service-role secrets MUST stay server-side in `apps/web` and never appear in `apps/mobile/.env.local` — the comment in `apps/mobile/.env.local.example` records the rationale.

**Deferred items (Phase Mobile)**

- **Shared Tailwind preset.** Extraction to `packages/shared` deferred to Phase Mobile M5 (documents list — when there's actual shared visual surface to justify the abstraction). Brand colours `navy` and `teal` are duplicated in `apps/web/tailwind.config.ts` and `apps/mobile/tailwind.config.ts`; inline comments in both flag the duplication.
- **iOS platform scaffold.** Deferred to a Mac session — see `apps/mobile/README.md` for the `cap add ios && cap sync ios && cap open ios` invocation. Linux/Codespace can author and bundle everything; only the iOS shell creation needs Xcode + CocoaPods.
- **Stock Capacitor splash + launcher icons.** Designer task for Phase Mobile M7 (native polish): 1024×1024 app icon master, splash screen art, Android adaptive icon foreground/background SVGs. Stock Capacitor "C" graphics ship through M2–M6.
- **Android test stub package rename.** `com.getcapacitor.myapp` → `au.com.sentinelhr.candidate` in `apps/mobile/android/app/src/{androidTest,test}/java/`. Cosmetic Capacitor template artefact; rename when actual Android tests get added (likely never in pilot).
- **APK build / device runs.** Codespace can't build an APK without a heavy Android SDK install and can't run an emulator (no KVM). Tracked as the "M2 device-verify" follow-up: blocked on a local machine with Android Studio for Android, and a Mac with Xcode for iOS.
...
- Candidate portal **Phase 3** in progress — `/portal/documents` (list + upload).
  ...
  - **M5 landed and verified end-to-end against dev** with a real 67 KB PDF upload by the seeded candidate: `documents` row created with full `<org>/<candidate>/<code>/<doc_id>.pdf` storage_path, status defaulted to `pending_review`, all NOT NULL columns populated; matching `audit_log` row with `action='documents.insert'`, `actor_type='candidate'`, and `entity_id` aligned with both `documents.id` and the storage filename (three sources of truth in lockstep). The full pipeline lives in `src/app/portal/documents/actions.ts`: `auth.getUser()` → `candidates` lookup by `user_id` → `document_types` lookup (system-global + active) → `documentUploadSchema(flags).safeParse` → `supabase.storage.upload()` (bucket `candidate-documents`, `upsert: false`) → `supabase.rpc('candidate_insert_document', ...)` → best-effort orphan-blob `.remove()` on RPC failure → SQLSTATE `23505` mapped to a friendly "already uploaded" message → `{ ok: true }` on success. `next.config.ts` lifts `experimental.serverActions.bodySizeLimit` to `"6mb"` (Next 16 only supports a global value — no per-route scoping today; the 5 MB PDF cap stays enforced in the validation schema and bucket config). Dialog client component (`upload-dialog.tsx`) gains a hidden `name="code"` input pinned to the row's doc-type code, plus a `useEffect` on `state.ok` (guarded by `didCloseRef`) that closes the modal and triggers a re-fetch — see M6 for the post-upload navigation refinement.
...
- **Phase 3 closed.**
- **Phase Mobile M1 closed** — workspace restructure ahead of the Capacitor wrap. Three commits: M1.1 scaffolding (root workspaces config + `tsconfig.base.json` + `packages/shared` skeleton); M1.2 moved the framework-neutral shared modules (`date`, `documents`, `validation/{auth,document,profile,au-states}`) into `@sentinel/shared` and rewrote the web app's imports; **M1.3 lifted the entire web tree under `apps/web/`** and split the root `package.json` into a workspace-orchestration manifest plus the moved `@sentinel/web` manifest. Each commit was verified end-to-end against dev (tsc + production build + 6-route smoke including a real PDF upload), so the branch was bisect-safe; fast-forwarded to main.
- **Phase Mobile M2 closed** — Capacitor scaffold + Hello-candidate placeholder. Five commits on branch `phase-mobile/m2-capacitor-scaffold`: M2.1 scaffolded `@sentinel/mobile` (package.json, tsconfig, next.config with `output: 'export'`, capacitor.config with `appId: au.com.sentinelhr.candidate`, `webDir: 'out'`); M2.2 added Tailwind/PostCSS/globals.css/layout shell with a system-font stack and a navy/teal-only Tailwind config (preset extraction deferred to M5); M2.3 replaced the placeholder `page.tsx` with a `"use client"` Hello-candidate page that exercises `formatAuDate` from `@sentinel/shared/date` inside `useEffect` (a bundled-JS wiring test, not a build-time-baked string — a future "optimisation" back to RSC would silently break the smoke signal); M2.4 ran `npx cap add android` to land the Android scaffold (53 trackable files, 408 KB; `appId` propagated end-to-end into `MainActivity`, `strings.xml`, and the generated `capacitor.config.json`); M2.5 added README + `.env.local.example` and ran the 7-step smoke test (build clean, sync clean, doctor green for Android, bundle 760 KB, largest JS chunk 223 KB, zero `service_role` hits, git tree clean). The done bar was bundle pipeline + anon-key invariant; device runs (Android emulator on local KVM; iOS Simulator on Mac) are tracked as a separate "M2 device-verify" follow-up.
- Next: two parallel tracks. **Phase 4 (web) — HR admin login flow + `organisation_members` seed**, then HR-side document review (verify / reject), then a candidate "view what I uploaded" affordance (signed-URL plumbing shared with HR review). And **Phase Mobile M3 — auth + session persistence** in the mobile bundle (`@supabase/supabase-js` with localStorage, candidate sign-in, redirect to a placeholder portal). Phase 4 lives entirely in `apps/web` and is unaffected by the mobile track. Re-upload / supersession via `replaces_document_id` is still deferred — the RPC currently always inserts as a new document and the unique partial index fires `23505` on a second attempt for the same type (mapped to a friendly message in the action). Per-org custom doc types, image upload support, and server-side virus scanning all remain post-MVP — though image upload becomes a v1-mobile dependency and is expected to widen the schema during Phase Mobile M6.
